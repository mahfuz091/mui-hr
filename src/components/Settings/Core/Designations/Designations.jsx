...
const Designations = () => {
  ...
  const handleDesignationAdd = async (event) => {
    ...
    try {
      ...
      toast.success("Designation has been successfully created.");
      handleClose();
      setControl(!control);
    } catch (error) {
      console.log(error);
  ...
  const handleDesignationDelete = async (id) => {
    console.log(id);
    try {
      const response = await axiosSecure.delete(`/api/designations/${id}`);
      console.log(response);

      if (response) {
        toast.success("Designation has been successfully deleted.");
      }
      setControl(!control);
    } catch (error) {
      console.log(error);
