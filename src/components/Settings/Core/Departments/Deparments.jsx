...
const Deparments = () => {
  ...
  const handledepartmentAdd = async (event) => {
    ...
    try {
      const response = await axiosSecure.post("/api/departments", dept);
      console.log(response);
      toast.success("Department has been successfully created.");
      handleClose();
      setControl(!control);
    } catch (error) {
      console.log(error);
  ...
  const handleDepartmentDelete = async (id) => {
    console.log(id);
    try {
      const response = await axiosSecure.delete(`/api/departments/${id}`);
      console.log(response);
    ...
    <Box>
      ...
      >
        <Box sx={style}>
          ...
          <Box sx={{ padding: "20px" }}>
            <form action='' onSubmit={handledepartmentAdd}>
              <Box>
                <TextField
                  fullWidth
                  margin='normal'
